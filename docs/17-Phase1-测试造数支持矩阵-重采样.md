...
|---|---:|---:|---:|---:|---:|
| web_common250 (无方言) | 250 | 250 | 196 | 54 | 78.4% |
| online_random250 (有方言) | 250 | 231 | 154 | 96 | 61.6% |

## 状态分布

- web_common250：`{"PASS": 196, "MISSED_COUNTEREXAMPLE": 26, "TACTIC_BUT_NO_COUNTEREXAMPLE": 28}`
- online_random250：`{"TACTIC_BUT_NO_COUNTEREXAMPLE": 66, "PASS": 154, "EXEC_ERROR": 19, "MISSED_COUNTEREXAMPLE": 11}`

## 按结构统计

| SQL 结构 | web 反例/总数 | online 反例/总数 | 当前结论 |
|---|---:|---:|---|
| SELECT | 11/11 | 14/15 | 稳定支持 |
| DISTINCT | 11/11 | 10/15 | 中等，复杂组合仍会掉点 |
| WHERE | 8/11 | 7/15 | 短板，需继续定向修复 |
| Comparison | 9/11 | 0/0 | 中等，复杂组合仍会掉点 |
| NULL | 6/10 | 0/0 | 中等，复杂组合仍会掉点 |
| IN / BETWEEN / LIKE | 18/30 | 0/0 | 中等，复杂组合仍会掉点 |
| Logic | 10/10 | 0/0 | 稳定支持 |
| JOIN | 7/11 | 0/0 | 中等，复杂组合仍会掉点 |
| JOIN ON | 11/11 | 9/15 | 中等，复杂组合仍会掉点 |
| GROUP BY | 11/11 | 7/15 | 中等，复杂组合仍会掉点 |
| HAVING | 10/10 | 10/15 | 中等，复杂组合仍会掉点 |
| Aggregate | 11/11 | 8/15 | 中等，复杂组合仍会掉点 |
| ORDER BY | 10/10 | 11/15 | 中等，复杂组合仍会掉点 |
| LIMIT / OFFSET | 10/10 | 11/15 | 中等，复杂组合仍会掉点 |
| Subquery | 5/10 | 6/15 | 短板，需继续定向修复 |
| Correlated Subquery | 5/10 | 5/14 | 短板，需继续定向修复 |
| CTE | 6/10 | 8/15 | 短板，需继续定向修复 |
| Recursive CTE | 10/10 | 10/14 | 中等，复杂组合仍会掉点 |
| Set Operation | 10/10 | 12/14 | 稳定支持 |
| CASE | 10/11 | 9/14 | 中等，复杂组合仍会掉点 |
| Window | 7/11 | 10/15 | 中等，复杂组合仍会掉点 |
| Dialect Boundary | 0/10 | 7/14 | 短板，需继续定向修复 |

## 真实完整样例

### 支持样例：嵌套 COUNT(DISTINCT) 可穿透
- 数据集：`web_common250` / 结构：`DISTINCT` / ID：`web_distinct_aggregate_gap_8`
...
学生:
```sql
SELECT COUNT(subject) FROM tickets;
```
- 造数状态：`PASS`
...
- 无方言组执行 250/250，反例穿透 196/250。
- 有方言组执行 231/250，反例穿透 154/250。
