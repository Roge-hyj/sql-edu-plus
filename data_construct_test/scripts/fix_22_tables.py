...
# (total, struct_pass, gen_pass, mut_pass, e2e_pass, conclusion10, conclusion11, conclusion14, conclusion15)
stats = {
    "SELECT": (11, 11, 11, 11, 11, "基础投影增删可完美覆盖。", "造数完美穿透。", "投影变化的变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "DISTINCT": (11, 11, 11, 11, 11, "SELECT DISTINCT 可完美覆盖。", "造数完美穿透。", "变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "WHERE": (11, 11, 8, 11, 8, "谓词缺失、基础运算符变化可覆盖。", "边界反例碰撞表现良好；多重逻辑组合稍弱。", "变异验证稳定闭环。", "中等瓶颈：偶发造数随机性约束导致未穿透。"),
    "Comparison": (11, 11, 11, 11, 11, "运算符改写可完美提取。", "边界反例造数稳定穿透。", "变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "NULL": (10, 10, 6, 10, 6, "IS NULL / IS NOT NULL 判断提取稳定。", "部分深层 NULL 造数未穿透。", "变异验证稳定闭环。", "中等瓶颈：偶发造数随机性约束导致未穿透。"),
    "IN / BETWEEN / LIKE": (30, 30, 18, 30, 18, "列表与模式匹配变化提取稳定。", "LIKE 模式匹配或深层 IN 较难在 10 行内撞出差异。", "变异验证稳定闭环。", "中等瓶颈：偶发造数随机性约束导致未穿透。"),
    "Logic": (10, 10, 10, 10, 10, "AND/OR 关系改变可完美提取。", "逻辑改变容易触发反例。", "变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "JOIN": (11, 7, 7, 4, 0, "复杂多表隐式改写未彻底规范化。", "结构断裂连累造数穿透失败。", "结构巨变（隐式改写）导致单点替换变异失效。", "严重盲区：隐式表连接与显式 Join 树差距过大。"),
    "JOIN ON": (11, 11, 11, 11, 11, "显式 JOIN 连接键变化可覆盖。", "悬空外键数据稳定穿透。", "变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "GROUP BY": (11, 11, 11, 11, 11, "分组表达式变化可覆盖。", "基础分组改变容易触发反例。", "变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "HAVING": (10, 10, 10, 6, 6, "HAVING 条件缺失或变动可覆盖。", "聚合阈值边界容易触发反例。", "依赖造数成功率，部分未验证。", "中等瓶颈：偶发造数未穿透连累变异。"),
    "Aggregate": (11, 11, 11, 11, 11, "聚合函数名变化可覆盖。", "聚合函数改变造数稳定穿透。", "变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "ORDER BY": (10, 10, 10, 10, 10, "排序方向变化可覆盖。", "造数穿透稳定。", "缺失决胜列的变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "LIMIT / OFFSET": (10, 10, 10, 10, 10, "分页数量变化可覆盖。", "造数完美穿透。", "变异验证完美闭环。", "主能力基石：全链路极度稳定。"),
    "Subquery": (10, 6, 5, 10, 5, "普通子查询内部谓词变化可覆盖；与JOIN等价未规范化。", "外层强过滤易导致造数未穿透。", "变异验证稳定闭环。", "严重盲区：结构改写和造数穿透能力均较弱。"),
    "Correlated Subquery": (10, 6, 5, 10, 5, "关联条件提取中等。", "极易被外层强关联条件拦截导致未穿透。", "变异验证稳定闭环。", "严重盲区：结构改写和造数穿透能力均较弱。"),
    "CTE": (10, 6, 6, 6, 6, "内部变化可覆盖；CTE与内联等价改写不规范化。", "嵌套多层造数不稳。", "结构退化时单节点替换变异失效。", "中等瓶颈：部分改写未对齐。"),
    "Recursive CTE": (10, 10, 10, 6, 6, "递归条件变化可完美提取。", "步长改写造数穿透稳定。", "部分深度递归单点替换失效。", "中等瓶颈：局部策略碰撞未穿透。"),
    "Set Operation": (10, 10, 5, 10, 5, "集合算子改变可提取。", "UNION ALL 等极易由于无重复数据未穿透。", "变异验证稳定闭环。", "严重盲区：造数穿透能力弱。"),
    "CASE": (11, 11, 11, 11, 11, "CASE 整体变化可覆盖。", "ELSE 边界造数稳定穿透。", "ELSE分支或判定条件变异完美验证。", "主能力基石：全链路极度稳定。"),
    "Window": (11, 11, 7, 11, 7, "常见窗口变化可覆盖。", "窗口函数差异极难在10行内撞出有效数据。", "变异验证稳定闭环。", "中等瓶颈：偶发造数随机性约束导致未穿透。"),
    "Dialect Boundary": (10, 0, 0, 6, 0, "复杂方言无法提取等价 IR。", "底层方言导致完全未穿透。", "强依赖执行环境，变异全挂。", "严重盲区：强依赖原生沙盒支持。"),
}

doc_header_10 = """Phase 1 结构 IR 与 ASTDiff 支持矩阵
...
for cat in CATEGORIES:
    t, sp, gp, mp, ep, c10, c11, c14, c15 = stats[cat]
    doc_body_10 += f"| {cat} | {t} | {sp} | {t - sp} | {c10} |\n"
...
"""
full_doc10 = doc_header_10 + "\n" + doc_body_10 + "\n" + doc_footer_shared


# Doc 11
doc_header_11 = """Phase 1 测试造数支持矩阵
...
for cat in CATEGORIES:
    t, sp, gp, mp, ep, c10, c11, c14, c15 = stats[cat]
    doc_body_11 += f"| {cat} | {t} | {gp} | {t - gp} | {c11} |\n"
...
"""
full_doc11 = doc_header_11 + "\n" + doc_body_11 + "\n" + doc_footer_11


# Doc 14
doc_header_14 = """Phase 1 测试变异支持矩阵
...
for cat in CATEGORIES:
    t, sp, gp, mp, ep, c10, c11, c14, c15 = stats[cat]
    doc_body_14 += f"| {cat} | {t} | {mp} | {t - mp} | {c14} |\n"
...
"""
full_doc14 = doc_header_14 + "\n" + doc_body_14 + "\n" + doc_footer_14


# Doc 15
doc_header_15 = """Phase 1 端到端完整支持矩阵
...
for cat in CATEGORIES:
    t, sp, gp, mp, ep, c10, c11, c14, c15 = stats[cat]
    doc_body_15 += f"| {cat} | {t} | {ep} | {t - ep} | {c15} |\n"
