...
# Data for 22 categories
# format: key: (supp_title, std, stu, doc10_cur, doc10_con, doc11_cur, doc11_con, doc14_cur, doc14_con, doc15_cur, doc15_con, unsupp_title, u_std, u_stu, u_doc10_cur, u_doc10_con, u_doc11_cur, u_doc11_con, u_doc14_cur, u_doc14_con, u_doc15_cur, u_doc15_con)
...
data = {
    ...
    "JOIN ON": {
        "supp": ("连接键写错", "SELECT * FROM a JOIN b ON a.id = b.id;", "SELECT * FROM a JOIN b ON a.id = b.val;"),
        "unsupp": ("ON 与 WHERE 过滤下推", "SELECT * FROM a LEFT JOIN b ON a.id = b.id AND b.val = 1;", "SELECT * FROM a LEFT JOIN b ON a.id = b.id WHERE b.val = 1;"),
    ...
    "GROUP BY": {
        ...
        "unsupp": ("主键函数依赖等价", "SELECT id, COUNT(*) FROM users GROUP BY id;", "SELECT id, name, COUNT(*) FROM users GROUP BY id, name;"),
    ...
    "Recursive CTE": {
        "supp": ("递归步长错误", "WITH RECURSIVE t AS (SELECT 1 n UNION ALL SELECT n+1 FROM t WHERE n<5) SELECT * FROM t;", "WITH RECURSIVE t AS (SELECT 1 n UNION ALL SELECT n+2 FROM t WHERE n<5) SELECT * FROM t;"),
        "unsupp": ("复杂多重锚点失效", "WITH RECURSIVE t AS (SELECT 1 n UNION SELECT 2 UNION ALL SELECT n+1 FROM t WHERE n<5) SELECT * FROM t;", "..."),
...
# --- DOCUMENT 10 ---
def build_doc(doc_type, header, cols):
    lines = [header]
    ...
    for cat in CATEGORIES:
        ...
        lines.append(f"标准：{info['supp'][1]}\n")
        lines.append(f"学生：{info['supp'][2]}\n")
        lines.append(f"当前表现：{info[doc_type]['scur']}\n")
        lines.append(f"结论：{info[doc_type]['scon']}\n\n")
    ...
    for cat in CATEGORIES:
        ...
        lines.append(f"标准：{info['unsupp'][1]}\n")
        lines.append(f"学生：{info['unsupp'][2]}\n")
        lines.append(f"当前表现：{info[doc_type]['ucur']}\n")
        lines.append(f"结论：{info[doc_type]['ucon']}\n\n")
        
    return "\n".join(lines)


with open(DOCS_DIR / "10-Phase1-结构IR与ASTDiff支持矩阵.md", "r", encoding="utf-8") as f:
    text10 = f.read()
    hdr10 = text10.split("可以支撑“目前支持”的样例")[0].strip()
    
with open(DOCS_DIR / "11-Phase1-测试造数支持矩阵.md", "r", encoding="utf-8") as f:
    text11 = f.read()
    hdr11 = text11.split("可以支撑“目前支持”的样例")[0].strip()

with open(DOCS_DIR / "14-Phase1-测试变异支持矩阵.md", "r", encoding="utf-8") as f:
    text14 = f.read()
    hdr14 = text14.split("可以支撑“目前支持”的样例")[0].strip()

with open(DOCS_DIR / "15-Phase1-端到端完整支持矩阵.md", "r", encoding="utf-8") as f:
    text15 = f.read()
    hdr15 = text15.split("可以支撑“目前支持”的样例")[0].strip()

with open(DOCS_DIR / "10-Phase1-结构IR与ASTDiff支持矩阵.md", "w", encoding="utf-8") as f:
    f.write(build_doc("doc10", hdr10, []))

with open(DOCS_DIR / "11-Phase1-测试造数支持矩阵.md", "w", encoding="utf-8") as f:
    f.write(build_doc("doc11", hdr11, []))

with open(DOCS_DIR / "14-Phase1-测试变异支持矩阵.md", "w", encoding="utf-8") as f:
    f.write(build_doc("doc14", hdr14, []))

with open(DOCS_DIR / "15-Phase1-端到端完整支持矩阵.md", "w", encoding="utf-8") as f:
    f.write(build_doc("doc15", hdr15, []))
